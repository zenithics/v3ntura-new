...
sleep 3
git clone "https://x-access-token:${GITHUB_TOKEN}@github.com/zenithics/$PROJECT_SLUG.git"
echo "   ✅ Repo cloned"
...
if [ "$USE_ECOM" = "y" ] || [ "$USE_ECOM" = "Y" ]; then
  ...
  git clone --depth 1 "https://x-access-token:${GITHUB_TOKEN}@github.com/zenithics/zenithics-ecom-addon.git" /tmp/ecom-addon
...
if [ "$USE_MULTISITE" = "y" ] || [ "$USE_MULTISITE" = "Y" ]; then
  ...
  git clone --depth 1 "https://x-access-token:${GITHUB_TOKEN}@github.com/zenithics/zenithics-multisite-addon.git" /tmp/multisite-addon
